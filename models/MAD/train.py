...
def inference(args, model, tokenizer, processor, split, step):
    ...
    if args.debug: 
        import pdb; pdb.set_trace()
        logger.info(preds)
        logger.info(softmax_preds)
        
    curr_result = compute_cls_metrics(preds, out_label_ids)
    
    # save
    
    loaded_data=pd.read_csv('data/preprocessed/movie_synopsis_segments_n3_after.csv')
    assert len(preds)==len(loaded_data)
    loaded_data[args.task_name]=softmax_preds
    loaded_data.to_csv('data/preprocessed/movie_synopsis_segments_n3_after.csv',index=False)
    ...
    # Log metrics
    output_eval_file = os.path.join(args.output_dir, 'inference_results_{}_{}_n3.txt'.format(args.task_name, split))
    with open(output_eval_file, 'a') as writer:
    ...
    # Log predictions
    output_pred_file = \
        os.path.join(args.output_dir, 'inference_{}_{}_{}_{}_n3.lst'.format(args.model_type, args.task_name, split, step))
    with open(output_pred_file, 'w') as writer:
        logger.info('***** Write predictions *****')
    ...
    # Maintain a single metrics file
    if os.path.exists(args.output_dir):
        with open(os.path.join(args.output_dir, 'inference_metrics_{}_{}_n3.json'.format(args.task_name, split)), 'w') as f:
            f.write(json.dumps(results))
        f.close()
...
def main():
    ...
    wandb.init(config=args, project="ModerN")
    ...
    args.data_dir = os.path.join(args.data_dir, "moral_stories_dataset")
